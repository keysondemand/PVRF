...
TEMPLATE_TEST_CASE("glow_dvrf_functions", "[benchmark][glow]", CryptoMcl, CryptoRelic) {
...

  {
    ...
    BENCHMARK_ADVANCED("Partial Eval - original GLOW ")(Catch::Benchmark::Chronometer meter) {
        auto i = static_cast<uint32_t>(rand() % committeeSize);
        std::string message{SHA512("hello" + std::to_string(rand() * rand())).toString()};
        //auto maskedinput = Drb::genMaskedInput(generator, message);
        meter.measure([&message, &outputs, &generator, i] {
          ...
          // At the VRF node, this verifies the masked input zkp and generates a new proof of correctness after exponentiating 
          //auto pr = Drb:: proofAfterPrivateInput(generator, message, get<1>(maskedinput), get<2>(maskedinput), get<3>(maskedinput), outputs[i].privateKey, outputs[i].publicKeyShares[i]);

          auto pi = Drb::proof(generator, message, outputs[i].publicKeyShares[i], sig, outputs[i].privateKey);
  ...
  } */
  {
    ...
    BENCHMARK_ADVANCED("Partial Eval - Output Private VRF - Generating input")(Catch::Benchmark::Chronometer meter) {
        auto i = static_cast<uint32_t>(rand() % committeeSize);
        std::string message{SHA512("hello" + std::to_string(rand() * rand())).toString()};
        meter.measure([&message, &outputs, &generator, i] {
          //auto sig = Drb::sign(message, outputs[i].privateKey);
  ...
  }
  {
    ...
    BENCHMARK_ADVANCED("Partial Eval - Outuput Private VRF - Proof at server ")(Catch::Benchmark::Chronometer meter) {
        auto i = static_cast<uint32_t>(rand() % committeeSize);
        std::string message{SHA512("hello" + std::to_string(rand() * rand())).toString()};
        auto maskedinput = Drb::genMaskedInput(generator, message);
        meter.measure([&message, &outputs, &generator, i, maskedinput] {
          //auto sig = Drb::sign(message, outputs[i].privateKey);
