...
public:
  ...
  //adding private signature 
  //using PrivateInput  = typename std::pair < MessagePayload , Signature>;
 ...
 */
  ...
  /*
  static PrivateInput genMaskedInput(const MessagePayload &message, const PrivateKey &privateKey) {
    Signature PH;
    Signature sign;
    PH.hashAndMap(message);
    sign.mult(PH, privateKey);
    return std::make_pair(message,sign);
  }
   */
