...
public:
  ...
  //using PrivateInput = typename GlowDkg<CryptoType>::PrivateInput;
  using PrivateInput  = typename std::pair <Signature,Proof>;

  //const PrivateKey localRand;
  //static VerificationKey expRand, maskedIn; 
  ...
  /*
  bool verifyProofOfMasked(const VerificationKey &y, const Signature &maskedIn, const Signature &sign,
    ...
    tmp.mult(y, tmps);
    c1.add(c1, tmp);
    c2.mult(maskedIn, proof.second);
    tmp.mult(sign, tmps);
    c2.add(c2, tmp);

    PrivateKey ch_cmp;
    ch_cmp.setHashOf(G, maskedIn, y, sign, c1, c2);
